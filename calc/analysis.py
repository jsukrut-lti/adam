...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Button(id='export-button-state', className='btnExport', children="Download Output", style={'display': 'none'})
...
def create_or_update_record(save=True,**kwargs):
    ...
    if input_data.get('action') == 'edit':
        ...
        rate_analysis_update_rec = RateAnalysis.objects.filter(pk=int(input_data.get('rate_analysis_id'))).update(**parent_data)
            ...
            if 'Total' not in filtered_var.get('APC change %',False):
                for line in inline_data:
                    ...
                    if is_present_list and max(is_present_list) == 1 or False:
                        ...
                        apc_perc_val = line.get('apc_change_perc',False) and filtered_var.get(line.get('apc_change_perc',False),False) or False
                        if apc_perc_val:
